...
public class PassthroughCameraTest : MonoBehaviour
{
    ...
    void DisablePassthroughCompositionLayer()
    {
            ...
            if (root.name == "Passthrough")
            {
                ...
                foreach (var component in root.GetComponents<Behaviour>())
                {
                    if (component.GetType().Name.Contains("CompositionLayer"))
                    {
                        ...
                        Debug.Log($"[PassthroughTest] Component '{component.GetType().FullName}' op 'Passthrough' UITGESCHAKELD.");
                    }
                }
                ...
                if (!disabledAComponent)
                {
                    ...
                    Debug.Log("[PassthroughTest] Geen uitschakelbaar CompositionLayer-component gevonden; " +
        ...
        Debug.LogWarning("[PassthroughTest] Geen 'Passthrough' root-object gevonden om uit te schakelen.");
    }
    ...
    void LogAllRootObjects()
    {
        ...
        Debug.Log($"[PassthroughTest] Aantal root-objecten in scene '{scene.name}': {rootObjects.Length}");
        foreach (var root in rootObjects)
        {
            Debug.Log($"[PassthroughTest] Root-object: '{root.name}', actief={root.activeSelf}, componenten: " +
            ...
            {
                Debug.Log($"[PassthroughTest]   Child: '{child.name}', componenten: " +
